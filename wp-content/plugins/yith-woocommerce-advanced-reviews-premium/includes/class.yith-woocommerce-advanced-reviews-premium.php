...
class YITH_WooCommerce_Advanced_Reviews_Premium extends YITH_WooCommerce_Advanced_Reviews {
	...
	public function initialize_settings() {
		...
		/** APPLY_FILTER: yith_ywar_vote_system_enabled
		*
		...
		*/
		...
		/** APPLY_FILTER: ywar_enable_reply_for_review_filter
		*
	...
	public function user_can_submit_review( $product_id, $review_id = 0 ) {
		...
		/** APPLY_FILTER: yith_advanced_reviews_user_can_submit_review_use_custom
		*
		...
		*/
		if ( apply_filters( 'yith_advanced_reviews_user_can_submit_review_use_custom', false, $product_id, $review_id ) ) {
			/** APPLY_FILTER: yith_advanced_reviews_user_can_submit_review_custom_value
			*
		...
		$can_submit =
			...
			/** APPLY_FILTER: ywar_wc_customer_bought_product_calls
			*
			...
			*/
			...
			/** APPLY_FILTER: yith_advanced_reviews_on_product_review_shown
			*
			...
			*/
			...
			/** APPLY_FILTER: yith_advanced_reviews_on_product_review_shown_denied
			*
			* Filter the condition to show the form in the frontend.
			*
			* @param bool false       Default false.
	...
	public function get_review_reply_link( $comment_id, $review, $product_id ) {
		...
		/** APPLY_FILTER: yith_advanced_reviews_allow_reply_condition
		*
	...
	public function show_expanded_review_content( $review ) {
			...
			if ( isset( $review_thumbnails ) && is_array( $review_thumbnails ) && ( count( $review_thumbnails ) > 0 ) ) {
				...
				foreach ( $review_thumbnails as $thumb_id ) {
					...
					if ( is_int( $thumb_id ) ) {
						...
						/** APPLY_FILTER: yith_ywar_review_thumbnail_size
						*
		...
		if ( $this->enable_review_vote && ( ! is_admin() || ( defined( 'DOING_AJAX' ) && DOING_AJAX ) ) ) {
			...
			if ( ! is_user_logged_in() && ! $this->enable_guest_vote ) {
				...
				/** APPLY_FILTER: yith_ywar_login_url
				*
				* Filter the condition to show the form in the frontend.
				*
		...
		/** APPLY_FILTER: yith_advanced_reviews_review_content_elements
		*
	...
	public function show_filter_helpful_recent( $product_id, $review_stats ) {
		...
		/** APPLY_FILTER: yith_ywar_show_filter_helpful_recent
		*
	...
	public function add_filtering_link( $stars, $product_id, $perc ) {
		if ( isset( $perc ) && ( $perc > 0 ) ) {
			/** APPLY_FILTER: yith_ywar_add_filtering_link
			*
	...
	public function enqueue_resource_frontend() {
		...
		wp_register_script(
			'ywar-script',
			/** APPLY_FILTER: yith_wc_advance_reviews_ywar_script
			*
		...
		wp_localize_script(
			...
			array(
				...
				/** APPLY_FILTER: yith_ywar_review_section_id
				*
				...
				*/
				...
				/** APPLY_FILTER: yith_ywar_add_review_title
				*
		...
		/** APPLY_FILTER: yith_advanced_reviews_enqueue_public_resources
		*
	...
	public function vote_review_callback() {
		...
		/** APPLY_FILTER: yith_ywar_check_ajax_wp_nonce_vote_review_callback
		*
	...
	public function get_review_is_helpful_text( $review_id ) {
		...
		return sprintf( esc_html__( '%1$s of %2$s people found this review helpful', 'yith-woocommerce-advanced-reviews' ), $vote['yes'], $vote['yes'] + $vote['not'] );
	}
	...
	public function get_ajax_comments_callback() {
		...
		ob_start();
		/** APPLY_FILTER: yith_advanced_reviews_reviews_list
		*
		...
		*/
		...
		if ( isset( $stars ) && ( $stars > 0 ) ) {
			...
			/** APPLY_FILTER: yith_advanced_reviews_reviews_title_text
			*
	...
	public function reviews_list( $product_id, $args = null, $show_featured = false ) {
		...
		/** APPLY_FILTER: 'yith_advanced_reviews_query_review_list_args',
		*
	...
	public function ywar_product_reviews_submit_reviews_denied_text( $message, $product_id ) {
		if ( $this->is_reviews_limited() && $this->customer_reviews_for_product( $product_id ) ) {
			...
			if ( $this->allow_edit_review ) {
				...
				/** APPLY_FILTER: 'ywar_denied_text_edit_allowed',
				*
	...
	public function yith_ywar_show_reviews( $atts, $content ) {
		...
		if ( isset( $atts['product_id'] ) ) {
			...
			foreach ( $reviews_aux as $review ) {
				...
				/** APPLY_FILTER: 'yith_ywar_show_review_shortcode_without_product_id',
				*
	...
	public function yith_ywar_add_endpoint() {
		...
		/** APPLY_FILTER: 'yith_ywar_reviews_endpoint_my_account',
		*
	...
	public function yith_ywar_add_reviews_endpoint_my_account( $items ) {
		...
		*/
		...
		/** APPLY_FILTER: 'yith_ywar_my_account_menu_item_title',
		*
